# Main Test con Sweep del Rischio (Mensile)
import numpy as np
...
from risk_performance_comparison import summarize_cumulative_returns_monthly
...
for lambda_risk_str, config_list in best_hyperparameters.items():
    ...
    for config_name in config_list:
        ...
        plot_grouped_bar_weights(f"{result_dir}/rebalance_weights_test.csv", f"{result_dir}/grouped_bar_weights_by_month.png")
        ...
        compute_monthly_metrics(ppo_log_returns, dates, save_path=f"{result_dir}/test_monthly_metrics.csv")
...
plt.grid(True)
plt.tight_layout()
plt.savefig(f"{BASE_RESULTS_DIR}/compare_lambda_risk_refined.png")
plt.close()

# === CSV RIEPILOGO ===
summarize_cumulative_returns_monthly(BASE_RESULTS_DIR)
