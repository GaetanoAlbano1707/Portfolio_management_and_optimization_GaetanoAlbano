...
for i in range(21, len(returns)):
    window_returns = returns.iloc[i - 21:i]
    cov_date = returns.index[i]
    cov_matrices[cov_date] = window_returns.cov()

# Salva le matrici di covarianza in un file pickle
with open("cov_matrices_monthly.pkl", "wb") as f:
    pickle.dump(cov_matrices, f)

print(f"✅ Generato {len(cov_matrices)} matrici di covarianza e salvato in 'cov_matrices_monthly.pkl'")
