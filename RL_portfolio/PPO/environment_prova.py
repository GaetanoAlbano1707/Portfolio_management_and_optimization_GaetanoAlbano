...
class PortfolioEnvProva(gym.Env):
    def __init__(self, df, tickers, cov_matrices=None, window_size=5, sharpe_window=50,
                 c_plus=0.001, c_minus=0.001, delta_plus=0.001, delta_minus=0.001,
                 rebalance_period=63, lambda_risk=0.1):
        super(PortfolioEnvProva, self).__init__()
        self.df = df
    ...
    def step(self, action):
        ...
        if is_rebalance_day:
            ...
            self.rebalance_weights.append(action.copy())
        ...
        # === Penalizzazione rischio
        cov_penalty = 0.0
        if self.cov_matrices is not None:
            date_index = self.df.index.get_level_values("date")[self.current_step]
            if date_index in self.cov_matrices:
                cov_matrix = self.cov_matrices[date_index]
                cov_penalty = np.dot(action, np.dot(cov_matrix, action))

        reward = log_return - cost - self.lambda_risk * cov_penalty
        if len(self.portfolio_returns) >= self.sharpe_window:
            ...
            reward = mean / std - cost - self.lambda_risk * cov_penalty
